...
class AnimatedSprite(GameSprite):
    ...
    def update(self, *args):
        if self.cur_animation >= len(self.animations):
            if (self.cur_frame == -1) or (self.cur_frame >= len(self.frames)):
                self.cur_frame = 0
                self.slowdown_step = 0
            self.slowdown_step += 1
            self.image = self.transform_frame(self.frames[self.cur_frame])
        else:
            if (self.cur_frame == -1) or (self.cur_frame >= len(self.animations[self.cur_animation])):
                self.cur_frame = 0
                self.slowdown_step = 0
            self.slowdown_step += 1
...
class MovableGameSprite(AnimatedSprite):
    def __init__(self, image: pygame.Surface, x: int, y: int, *group, level_map, box_service: BoxService, width=0,
                 height=0, hspeed=0, vspeed=800, jump_height=3 * commonConsts.BLOCK_SIZE, right_direction=False,
                 strongth=False):
    ...
    def fall(self):
        ...
        offset = 0
        if self.can_stay(self.level_map[self.get_bottom_cell_y(1)][self.get_left_cell_x()]) or self.can_stay(
                self.level_map[self.get_bottom_cell_y(1)][self.get_right_cell_x()]):
            return
    ...
    def move(self, can_fall=True, flip_on_stop=False, distance=0):
        if distance == 0:
            distance = self.hspeed // commonConsts.FPS
        if distance == 0:
            return
        ...
        while offset < distance:
            ...
            if self.right_direction:
                ...
                block_back = right
                block_front = left
            if not self.can_move(self.level_map[self.get_bottom_cell_y()][block_front]) or not self.can_move(
                    self.level_map[self.get_top_cell_y()][block_front]):
                if self.right_direction:
